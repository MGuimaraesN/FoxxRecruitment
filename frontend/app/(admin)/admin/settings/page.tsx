...
import { 
    ...
    Image as ImageIcon, LayoutTemplate, AlertCircle 
} from 'lucide-react';
...
export default function SettingsPage() {
    ...
    // Estado para controlar qual instituição está sendo editada
    const [targetId, setTargetId] = useState<number | null>(activeInstitutionId);
    
    // Lista de instituições (apenas para Super Admin na visão global)
    const [availableInstitutions, setAvailableInstitutions] = useState<InstitutionSimple[]>([]);
    ...
    // 1. Sincroniza targetId com a instituição ativa (se houver)
    useEffect(() => {
        if (activeInstitutionId) {
            setTargetId(activeInstitutionId);
        }
    }, [activeInstitutionId]);

    // 2. Se for Super Admin e estiver na visão global, busca lista de instituições
    useEffect(() => {
        const fetchList = async () => {
            if (!token || activeInstitutionId || !isSuperAdmin) return;
            try {
                // Endpoint que lista instituições (já existente no seu controller)
        ...
        fetchList();
    }, [token, activeInstitutionId, isSuperAdmin]);

    // 3. Carrega os dados da instituição alvo (targetId)
    useEffect(() => {
        const fetchSettings = async () => {
            if (!token || !targetId) {
                if (!activeInstitutionId && !isSuperAdmin) {
                    // Se não for super admin e não tiver ID, para o loading
                    setIsLoading(false);
                }
                // Se for super admin e ainda não selecionou, para o loading da página (mostra o select)
                if (!activeInstitutionId && isSuperAdmin && availableInstitutions.length > 0) {
                    setIsLoading(false);
                }
                return; 
            }
            ...
            setIsLoading(true);
            try {
                const res = await fetch(`${API_URL}/institutions/${targetId}`, {
                    headers: { Authorization: `Bearer ${token}` }
                });
                if (res.ok) {
                    const data = await res.json();
                    ...
                    setPreviewUrl(null); // Reseta preview anterior
                    setLogoFile(null);   // Reseta arquivo anterior
                }
            } catch (error) {
        ...
        fetchSettings();
    }, [token, targetId, availableInstitutions.length]); // Dependências ajustadas
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                // Se estiver editando a instituição ativa atual, atualiza o perfil global
                if (targetId === activeInstitutionId) {
                    fetchUserProfile();
                }

                setCurrentLogoUrl(updatedData.logoUrl);
                setPreviewUrl(null);
                setLogoFile(null);
            } else {
    ...
    if (isLoading && !targetId && !availableInstitutions.length) {
        return <div className="flex h-96 items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-blue-600"/></div>;
    }

    // Se não tiver ID alvo e não for Super Admin (ou lista vazia), mostra aviso
    if (!targetId && !isSuperAdmin) {
        return (
            <div className="flex flex-col items-center justify-center h-64 text-neutral-500 bg-white rounded-xl border border-neutral-200">
                <Building className="h-10 w-10 mb-2 opacity-20" />
                <p>Nenhuma instituição selecionada para configurar.</p>
            </div>
        );
    }
        ...
        <div className="max-w-4xl mx-auto space-y-8 pb-20">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-neutral-500 mt-1">Personalize a identidade visual do portal de vagas.</p>
                </div>

                {/* SELETOR PARA SUPER ADMIN (Visão Global) */}
                {!activeInstitutionId && isSuperAdmin && (
                    <div className="w-full md:w-[300px]">
                        <label className="text-xs font-semibold text-neutral-500 mb-1 block uppercase">Selecione para editar:</label>
                        <Select 
                            value={targetId?.toString() || ''} 
                            onValueChange={(val) => setTargetId(Number(val))}
                        >
                            <SelectTrigger className="bg-white">
                                <SelectValue placeholder="Escolha uma faculdade..." />
                            </SelectTrigger>
                            <SelectContent>
            ...
            {/* SE NENHUM ID SELECIONADO AINDA (Super Admin acabou de entrar) */}
            {!targetId ? (
                <div className="bg-blue-50 border border-blue-100 rounded-xl p-8 text-center flex flex-col items-center">
                    ...
                    <h3 className="text-lg font-semibold text-blue-900">Modo de Visão Global</h3>
                    <p className="text-blue-700 mt-1 max-w-md">
                        Você não está vinculado a uma instituição específica no momento. 
                        Selecione uma instituição na lista acima para editar suas configurações de branding.
                    </p>
                </div>
            ) : (
